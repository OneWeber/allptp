...
//获取旅途的异步action
export function onLoadTravel(storeName, url, data, isRefresh, oNum ,callBack) {
    return dispatch => {
        dispatch({type: Types.TRAVEL_REFRESH, storeName: storeName})
...
function handleData(dispatch, storeName, resData, dataNum) {
    dispatch({
        type: Types.LOAD_TRAVEL_SUCCESS,
        items: resData,
        storeName: storeName,
        dataNum: dataNum
    })
...
export function onLoadMoreTravel(storeName, url, data, oItems, callBack) {
    return dispatch => {
        dispatch({type: Types.MORE_TRAVEL_LOADING, storeName: storeName})
        let dataStore = new DataStore()
        dataStore.fetchData(url, 'POST', data).then(resData => {
            if(resData.data.data.data.length > 0){
                ...
                dispatch({
                    type: Types.ONLOAD_MORETRAVEL_FAIL,
                    storeName,
                    error
                })
            }
...
function handleDataMore(dispatch, storeName, resData) {
    dispatch({
        type: Types.ONLOAD_MORETRAVEL_SUCCESS,
        items: resData,
        storeName
    })
}
