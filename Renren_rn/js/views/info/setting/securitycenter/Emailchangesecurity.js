...
type Props = {};
class Emailchangesecurity extends Component<Props>{
    constructor(props) {
        super(props);
        var newbCodeInter=null;
        var newbCodeInters=null;
        this.state={
            token:'',
            userInfo: "",
            bCodeTel:false,
    ...
    componentWillUnmount(){
        clearInterval(this.newbCodeInter)
        clearInterval(this.newbCodeInters)
    }
    surePassword(){
        if(this.state.password!=this.state.repassword){
            ...
            Fetch.post(NewHttp+'FindPayPwd',formData).then(
                result=>{
                    if(result.code==1){
                        this.refs.toast.show('修改安全密码成功');
                        let _this=this;
                        this.newbCodeInters=setInterval(()=>{
                ...
                <View style={{flex:1,position:'relative'}}>
                    <View style={styles.calendarHeader}>
                        <View style={styles.calendarHeaderCon}>
                            ...
                            />
                            <Text style={{fontSize:16,color:"#333333"}}>邮箱修改安全密码</Text>
                                ...
                                <View style={{width:'100%',marginTop:25}}>
                                    <View style={{width:'100%',justifyContent:'flex-start',alignItems:'center',flexDirection:'row'}}>
                                        ...
                                        <View style={{marginLeft:15}}>
                                            <Text style={{fontSize:16,color:'#666666'}}>{this.state.userInfo.email_link}</Text>
                                        </View>
                                    </View>
                                </View>
...
export default connect(mapStateToProps)(Emailchangesecurity)
